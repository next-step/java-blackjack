...
public class BlackJackTest {
    ...
    @DisplayName("BlackJack 상태를 생성 할 수 있다")
    @Test
    void generateHit() {
        State state = new BlackJack(bunchOfCard);

        assertThat(state.getClass()).isEqualTo(BlackJack.class);
    }


    @DisplayName("BlackJack 상태는 게임이 끝난 상태이다.")
    @Test
    ...
    @DisplayName("BlackJack 보상은 베팅금액의 1.5배이다.")
    @Test
    void hitCanNotGetProfit() {
        State state = new BlackJack(bunchOfCard);

        assertThat(state.profit(1)).isEqualTo(1.5);
    }

    @DisplayName("BlackJack 상태에서 드로우를 할 수없다")
    @Test
    void keepHit() {
        State state = new BlackJack(bunchOfCard);
