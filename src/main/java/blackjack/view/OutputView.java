...
public class OutputView {
    ...
    private static final String DEAL_MESSAGE_FORMAT = "%s에게 카드를 %d장씩 분배하였습니다.%n";
    ...
    private static void printDealResult(final List<Participant> participants) {
        System.out.printf(DEAL_MESSAGE_FORMAT, getNames(participants), NUMBER_OF_INIT_HANDS);
    }
    ...
    public static void printParticipantsStatus(final List<Participant> participants) {
        ...
        participants.forEach(participant ->
            status.append(String.format(
                PARTICIPANT_STATUS_FORMAT, participant.getName(), getStartCardNames(participant))));

        System.out.println(status);
    }

    private static String getStartCardNames(final Participant participant) {
        if (participant instanceof Dealer) {
            return getOneCard(participant);
        }
        return getCardNames(participant);
    }

    private static String getOneCard(Participant participant) {
        return participant.getCardNames().get(FIRST_CARD);
    }
    ...
    private static void printFinalStatus(List<Participant> participants) {
        ...
        participants.forEach(participant ->
            status.append(String.format(
                PARTICIPANT_FINAL_STATUS_FORMAT,
                participant.getName(), getCardNames(participant), participant.getScore())));
